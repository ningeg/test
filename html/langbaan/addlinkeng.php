...
$sql = "select  *  from  link_tb_en   where l_id is not null order by c_position desc limit 1 ";
$res = DBexec($Dbs,$sql);
$rows = DBNumrows($res);
$newpos = 1 ; 
if($rows <> "0"){
...
$insert = "insert into link_tb_en  set l_name='$HTTP_POST_VARS[l_name]',l_url='$HTTP_POST_VARS[l_url]',l_logo='$thumb1_name',c_position='$newpos'";
DBexec($Dbs,$insert);
Header("Location:managelinkeng.php");
}
if($HTTP_POST_VARS[Update]){
$update = "update  link_tb_en   set  l_name='$HTTP_POST_VARS[l_name]',l_url='$HTTP_POST_VARS[l_url]'";
if($thumb1_name){
$update .=",l_logo='$thumb1_name'";
} 
$update .= " where l_id='$HTTP_POST_VARS[l_id]'";
DBexec($Dbs,$update);
...
<TABLE cellSpacing=2 cellPadding=2 width="100%" border=0>
  ...
  <TR>
    <TD bgColor=#cccccc><STRONG 
      class=font>&nbsp;&nbsp;&nbsp;เพิ่ม<span lang="th">ลิงค์พันธมิตรENG</span></STRONG></TD>
  </TR></TBODY></TABLE>
<FORM name=form1 action="" method=post enctype="multipart/form-data">
<?
			if($HTTP_GET_VARS[l_id]){
			$sql = "select * from link_tb_en  where  l_id='$HTTP_GET_VARS[l_id]'";
			  $res =  DBexec($Dbs,$sql);
			  $rows = DBNumrows($res);
			  $arr = DBfetch_array($res,0);
	}
