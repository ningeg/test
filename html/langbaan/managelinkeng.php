...
$delsql = "delete from link_tb_en where l_id='$HTTP_GET_VARS[l_id]'";
DBexec($Dbs,$delsql);
}
if($HTTP_GET_VARS[event] == "up"){


$up_sql = "select * from  link_tb_en   where l_id is not null  order by  c_position asc";
...
if($arr[c_position]  == $HTTP_GET_VARS[old] ){
...
$newpos = $i - 1 ; 
		...
		$update1 = "update  link_tb_en   set  c_position = '$newar[c_position]' where l_id='$arr[l_id]'";
		DBexec($Dbs,$update1); 			
			
			//********* Change position down *******
			
			$update2 = "update link_tb_en  set c_position ='$arr[c_position]' where  l_id='$newar[l_id]'";
			DBexec($Dbs,$update2);
...
<TABLE cellSpacing=2 cellPadding=2 width="100%" border=0>
  ...
  <TR>
    <TD bgColor=#cccccc><STRONG 
      class=font>&nbsp;&nbsp;&nbsp;รายชื่<span lang="th">อลิงค์พันธมิตรENG</span></STRONG></TD>
  </TR></TBODY></TABLE>
...
width=700 borderColorLight=#999999 border=1>
  ...
  <TR class=font bgColor=#60aad2>
	...
	if($Search){
 $sql= "select * from link_tb_en  where category_th like '%$text_search%'";
				  }else{

					
	$sql = "select * from link_tb_en  order by c_position  DESC";
	}
...
if($bg =="#FFFFFF"){
  ...
  <TR class=font bgcolor="<?=$bg;?>">
    ...
    <TD width="171">
      ...
      href="addlinkeng.php?l_id=<?=$arr[l_id]?>&edit=1"> 
