...
$delsql = "delete from newscenter3 where news_id='$HTTP_GET_VARS[news_id]'";
DBexec($Dbs,$delsql);
}
?>
<HTML><HEAD>
...
width=700 borderColorLight=#999999 border=1>
 ...
 $sql= "select * from newscenter3  where title_th like '%$HTTP_POST_VARS[text_search]%'";
				  }else{
...
if($bg =="#FFFFFF"){
			...
			if($arr[title_eng]==""){echo"";}else{
	
		?>
  <TR class=font>
    <TD width="405">
      <DIV class=detalbody align=center><?=$arr[title_eng]?> </DIV></TD>
   
    <TD width="171">
      <DIV class=detalbody align=center>&nbsp;<A 
      href="addnews3eng.php?news_id=<?=$arr[news_id]?>&edit=1"> 
