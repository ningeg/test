...
$insert = "insert into newscenter2 set title_eng='$HTTP_POST_VARS[title_eng]',title_th='$HTTP_POST_VARS[title_th]',details_eng='$HTTP_POST_VARS[details_eng]',details_th='$HTTP_POST_VARS[details_th]',pic_s='$thumb1_name',pic_b='$picture1name',file_name='$HTTP_POST_VARS[file_name]',file_down='$picture2_name',ddpost='".date("YmdHis")."'";
DBexec($Dbs,$insert);
Header("Location:managenews2.php");
}
if($HTTP_POST_VARS[Update]){
$update = "update newscenter2 set title_eng='$HTTP_POST_VARS[title_eng]',title_th='$HTTP_POST_VARS[title_th]',details_eng='$HTTP_POST_VARS[details_eng]',details_th='$HTTP_POST_VARS[details_th]'";
if($picture1name){
...
<TABLE cellSpacing=2 cellPadding=2 width="100%" border=0>
  ...
  <TR>
    <TD bgColor=#cccccc><STRONG 
      class=font>&nbsp;&nbsp;&nbsp;เพิ่มรายการ</STRONG></TD>
  </TR></TBODY></TABLE>
<FORM name=form1 action="" method=post enctype="multipart/form-data">
<?
			if($HTTP_GET_VARS[news_id]){
			$sql = "select * from newscenter2  where  news_id='$HTTP_GET_VARS[news_id]'";
			  $res =  DBexec($Dbs,$sql);
			  $rows = DBNumrows($res);
...
<TABLE cellSpacing=3 cellPadding=3 width=864 align=center border=0>
  ...
  <TR>
    ...
    <TD align="right"><INPUT id=title_th size=62 name=title_th value="<?=$arr[title_th];?>"  class="inputBox" style="width: 760px;"></TD></TR>
  <TR>
